...
- **Display is fatal, but trailing.** `display_init()` is `ESP_ERROR_CHECK`ed like every other init: the Menu, the Sync result, and Idle all live on the panel, so a device that cannot paint cannot be driven, and limping on with a blind UI hides that. It stays *ordered* after `start_capture()` on a Record-Button wake, because the panel bring-up is slow and on the happy path that time is audio clipped off the front of every memo -- so a panel failure does abort the Capture that had already begun, header unpatched. Accepted: there is no working device left to save that file for. Paints still trail the audio action (record task starts, then the screen paints; the WAV closes, then Idle paints), which is now purely about latency, not failure isolation.
