...
- [x] Light sleep in Idle **via automatic PM**, not a manual `esp_light_sleep_start()` (ADR 0001, ADR 0010). `CONFIG_PM_ENABLE` + `CONFIG_FREERTOS_USE_TICKLESS_IDLE` in `sdkconfig.defaults` (existing `sdkconfig` had to be regenerated -- defaults don't merge into a live one), then one `esp_pm_configure()` (`min==max==160`, DFS off, `light_sleep_enable=true`) after the init block, before the main loop. No per-state gating: the enabled I2S channel (Recording) and Wi-Fi (Syncing) each hold an APB-max PM lock that blocks sleep; both drop it when the state ends, so Idle is lock-free and sleeps on its own. Wake is already armed -- buttons are created `enable_power_save=true`, which stops the `iot_button` poll timer and arms `gpio_wakeup` on GPIO0/GPIO18 when no press is pending
  - [x] **Synchronous display port** (the real enabler). Deleted the 2ms periodic `esp_timer` (`lv_tick_inc`) and the background `lvgl_task` -- the 2ms tick alone forced ~500 wake/s and made the PM config a no-op. Each `display_show_*` now loads its screen and calls `lv_refr_now(display)` (not a hand-rolled `lv_timer_handler` loop -- `lv_refr_now` is LVGL's primitive for exactly this), so `flush_cb` runs inline on the calling task and blocks on the panel BUSY line. Tick pull-based via `lv_tick_set_cb` -> `esp_timer_get_time()`. `lvgl_mux` dropped (single owner once `lvgl_task` is gone)
    - **Gotcha that bit:** `display_init` loads the idle screen but nothing flushed it once `lvgl_task` was gone -- normal cold boot showed a blank panel. Fixed by ending `display_init` with `full_refresh_pending = true; lv_refr_now(display);`
  - [x] **`esp_sleep_enable_gpio_switch(false)`** in `app_main`. Light sleep defaults to isolating GPIOs during sleep (`sleep_gpio: Configure to isolate all GPIO pins`), which floats `EPD_PWR_PIN` (GPIO6, panel power gate, active-low -- and never explicitly driven, powered by reset-default luck). On the first Record-wake the panel had browned out, so `EPD_DisplayPart` hung on a BUSY line that never cleared -> `read_busy` spin -> task watchdog. Disabling the switch holds all GPIOs through sleep, covering EPD power and the SPI control pins at once
  - No `gpio_hold` plumbing needed for the audio-amp gate: light sleep auto-retains digital GPIO output state, so `Audio_PWR_PIN` (GPIO42) stays off through Idle on its own. (GPIO42 is not RTC-capable -- >21 -- so it couldn't be held or used as a wake source under deep sleep anyway; another point for light sleep. Wake pins GPIO0/GPIO18 are both RTC-capable.) GPIO17 (VBAT latch) likewise held (confirmed: device stays alive through sleep/wake)
  - [ ] **Verify on battery (USB tethering perturbs both):** inline current drops toward the ~1-2mA of ADR 0001; GPIO17 survives (alive + wakes on both buttons); recording round-trips cleanly (playback not pitch-shifted -> I2S rate correct under APB=160). `esp_pm_dump_locks()` shows no lock in Idle. `CONFIG_ESP_SLEEP_DEBUG=y` for sleep/wake transition logs
  - [ ] **Dev ergonomics:** reflashing while PM is on can fail auto-reset ("No serial data received") -- a sleeping chip won't take the USB-Serial-JTAG reset. Recover via manual download mode (hold BOOT/Record = GPIO0, tap EN, release). If it gets tedious, delay `esp_pm_configure()` a few seconds after boot so esptool can catch the chip awake
  - [ ] **`read_busy` has no timeout** (`epaper_driver_bsp.cpp:123`): any panel stall becomes an infinite loop, as the watchdog above showed. Bound the wait and surface a failure instead of hanging
