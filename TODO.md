...
- [ ] Light sleep in Idle **via automatic PM**, not a manual `esp_light_sleep_start()` (ADR 0001, ADR 0010). `CONFIG_PM_ENABLE` + `CONFIG_FREERTOS_USE_TICKLESS_IDLE`, then one `esp_pm_configure()` (`min==max==160`, DFS off, `light_sleep_enable=true`) after the init block, before the main loop. No per-state gating: the enabled I2S channel (Recording) and Wi-Fi (Syncing) each hold an APB-max PM lock that blocks sleep; both drop it when the state ends, so Idle is lock-free and sleeps on its own. Wake is already armed -- buttons are created `enable_power_save=true`, which stops the `iot_button` poll timer and arms `gpio_wakeup` on GPIO0/GPIO18 when no press is pending
  - **Blocker (do first): synchronous display port.** The lifted LVGL port runs a 2ms periodic `esp_timer` (`lv_tick_inc`) and a background `lvgl_task` forever; the 2ms tick alone forces ~500 wake/s and makes the PM config a no-op. Delete both. Drive each `display_show_*` synchronously: load screen, pump `lv_timer_handler()` until `flush_cb` signals done, return. Tick pull-based via `lv_tick_set_cb` -> `esp_timer_get_time()` (no running timer). UI never animates, so this is safe. Drop `lvgl_mux` too -- single owner (main task) once `lvgl_task` is gone
  ...
  - **Same retention covers GPIO17** (VBAT power-hold latch): auto-retained through light sleep, no hold needed -- but a drop cuts the battery rail and looks bricked, so **gate on a hardware test**: unplug USB, enter Idle light sleep, confirm alive + wakes on both buttons
  - **Verify it actually sleeps** (blocking on `app_events_wait` looks the same either way): `esp_pm_dump_locks()` shows no lock in Idle, and inline battery-current drops toward the ~1-2mA of ADR 0001
